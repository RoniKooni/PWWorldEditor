...
function restoreEditorState(state) {
    if (state.layers && state.layers.length) {
        editorLayers = state.layers.map(layer => ({
            id: layer.id,
        ...
        activeLayerId = state.activeLayerId || editorLayers[0].id;
        selectedLayerId = activeLayerId;
        layerSeq = state.layerSeq || (Math.max(...editorLayers.map(layer => layer.id)) + 1);
    } else {
...
function renderLayerPanel() {
    ...
    [...editorLayers].reverse().forEach(layer => {
        ...
        menuDot.title = layer.locked ? 'Locked layer' : 'Right-click for layer actions';
...
function duplicateLayer() {
    ...
    const layer = { id: layerSeq++, name: `${source.name} copy`, fg: cloneGrid(source.fg), bg: cloneGrid(source.bg), visible: true, locked: false };
...
function deleteLayer(targetLayerId) {
    ...
    const preferredId = Number.isFinite(requestedId) && requestedId > 0
        ? requestedId
        : (layerDeleteTargetId || selectedLayerId || activeLayerId);
    const targetId = editorLayers.some(item => item.id === preferredId) ? preferredId : activeLayerId;
...
function putBlockInHotbar(block) {
    ...
    const slot = document.querySelectorAll('.slot')[targetSlot];
...
function updateToolState(tool) {
    ...
    else if (tool === 'select') display.innerText = "SELECT";
    else if (tool === 'arrange') display.innerText = "ARRANGE LAYER";
...
document.querySelectorAll('[data-dock-target]').forEach(btn => {
    btn.onclick = () => showDockPanel(btn.dataset.dockTarget);
});
    ...
    entries.forEach(([name, data]) => {
        ...
        card.innerHTML = `
            ...
            <div style="flex:1;min-width:0;">
                ...
                <div style="font-size:13px;color:#fff;font-weight:700;margin-top:2px;">x ${total}</div>
...
document.getElementById('layer-context-menu')?.addEventListener('click', (e) => {
    ...
    if (action === 'arrange') setArrangeTool();
...
(function () {
    ...
    function drawSelection() {
        ...
        // Confirmed box selection. Arrange uses the layer directly and does not show a box.
        ...
        if (activeTool === 'arrange' && movePreview && liftedFg && sel) {
            const drawX = sel.x + movePreview.dx;
            const drawY = sel.y + movePreview.dy;
            selCtx.save();
            selCtx.globalAlpha = 0.65;
            for (let dx = 0; dx < liftedFg.length; dx++) {
                for (let dy = 0; dy < (liftedFg[0]||[]).length; dy++) {
    ...
    viewport.onmousedown = function(e) {
        ...
        if (activeTool === 'arrange') {
            const bounds = getLayerContentBounds(activeLayer());
    ...
    window.onmousemove = function(e) {
        ...
        if (activeTool === 'arrange' && isMoveDragging && sel && moveStart) {
            const t = toTile(e.clientX, e.clientY);
            const dest = clampRegionPos(sel.x + t.x - moveStart.x, sel.y + t.y - moveStart.y, sel.w, sel.h);
            movePreview = { dx: dest.x - sel.x, dy: dest.y - sel.y };
            const movingData = { fg: liftedFg, bg: liftedBg, w: sel.w, h: sel.h };
            viewport.style.cursor = canPlaceRegion(movingData, dest.x, dest.y, { x: sel.x, y: sel.y, w: sel.w, h: sel.h }) ? 'grabbing' : 'not-allowed';
            return;
        }
        ...
        if (activeTool === 'select' && sel && insideSel(mouseX, mouseY) && !isPastingMode) {
            viewport.style.cursor = 'move';
        } else if (activeTool === 'arrange' && getLayerContentBounds(activeLayer()) && isInsideRect(mouseX, mouseY, getLayerContentBounds(activeLayer()))) {
    ...
    window.onmouseup = function(e) {
        ...
        if (activeTool === 'arrange' && isMoveDragging && movePreview) {
            ...
            const dest = clampRegionPos(sel.x + dx, sel.y + dy, sel.w, sel.h);
            const movingData = { fg: liftedFg, bg: liftedBg, w: sel.w, h: sel.h };
            if (!canPlaceRegion(movingData, dest.x, dest.y, { x: sel.x, y: sel.y, w: sel.w, h: sel.h })) {
                cancelLift();
            ...
            dropLifted(dest.x - sel.x, dest.y - sel.y);
            sel = null;
            isMoveDragging = false; moveStart = null; movePreview = null;
            viewport.style.cursor = 'grab';
            return;
    ...
    window.onkeydown = function(e) {
        ...
        if (activeTool === 'select' || activeTool === 'arrange') {
            ...
            if (activeTool === 'arrange' && ['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)) {
                ...
                const dest = clampRegionPos(sel.x + ddx, sel.y + ddy, sel.w, sel.h);
                const movingData = { fg: liftedFg, bg: liftedBg, w: sel.w, h: sel.h };
                if (!canPlaceRegion(movingData, dest.x, dest.y, { x: sel.x, y: sel.y, w: sel.w, h: sel.h })) {
                    cancelLift();
                    return;
        ...
        if (deleteKey && !e.ctrlKey && !e.metaKey && !e.altKey) {
            e.preventDefault();
...
const refOverlayEl = (() => {
    ...
    el.style.cssText = 'position:absolute;top:50%;left:50%;pointer-events:none;z-index:10;transform-origin:top left;max-width:none;max-height:none;';
...
function updateRefOverlay() {
    ...
    refOverlayEl.style.transform = `translate(-50%, -50%) translate(${posX}px, ${posY}px) scale(${scale}) translate(${ox}px, ${oy}px) scale(${sc})`;
}

...
document.getElementById('file-input').onchange = (e) => {
    ...
    reader.onload = () => {
        ...
        fgData = d.fg; bgData = d.bg; setBackground(d.atm);
...
(function () {
    ...
    let clipboard = null;    // { fg:[][],  bg:[][] } w×h arrays
    ...
    function pasteRegion(data, destX, destY) {
        for (let dx = 0; dx < data.w; dx++) {
            for (let dy = 0; dy < data.h; dy++) {
                ...
                fgData[wx][wy] = data.fg[dx][dy] ? JSON.parse(JSON.stringify(data.fg[dx][dy])) : null;
                bgData[wx][wy] = data.bg[dx][dy] ? JSON.parse(JSON.stringify(data.bg[dx][dy])) : null;
            }
        }
    }
    ...
    function drawSelection() {
        ...
        if (sel) {
            ...
            selCtx.fillRect(drawX * TILE, drawY * TILE, sel.w * TILE, sel.h * TILE);
            ...
            selCtx.strokeRect(drawX * TILE, drawY * TILE, sel.w * TILE, sel.h * TILE);

            // Corner handles
            selCtx.setLineDash([]);
            selCtx.fillStyle = '#3abdc2';
            const H = 6;
            [[0,0],[sel.w,0],[0,sel.h],[sel.w,sel.h]].forEach(([ox,oy]) => {
                selCtx.fillRect((drawX + ox) * TILE - H/2, (drawY + oy) * TILE - H/2, H, H);
            });

            // Draw live preview of lifted blocks during move
            if (movePreview && liftedFg) {
                selCtx.globalAlpha = 0.55;
                for (let dx = 0; dx < liftedFg.length; dx++) {
                    for (let dy = 0; dy < (liftedFg[0]||[]).length; dy++) {
                        const wx = drawX + dx, wy = drawY + dy;
                        if (wx < 0 || wx >= GRID_X || wy < 0 || wy >= GRID_Y) continue;
                        const blk = liftedFg[dx][dy] || liftedBg[dx][dy];
                        if (!blk) continue;
    ...
    function showSelToolbar() { if (sel) selToolbar.classList.remove('hidden'); }
    function hideSelToolbar() { selToolbar.classList.add('hidden'); }

    function deselect() {
        if (isMoveDragging) cancelLift();
        sel = null; selDrag = null;
        isMoveDragging = false; moveStart = null; movePreview = null;
        isPastingMode = false;
        hideSelToolbar();
        selPasteBtn.style.display = 'none';
    }

    selCopyBtn.onclick = () => {
        if (!sel) return;
        clipboard = copyRegion(sel.x, sel.y, sel.w, sel.h);
        selPasteBtn.style.display = '';
    ...
    viewport.onmousedown = function(e) {
        if (activeTool !== 'select') { origMousedown && origMousedown.call(this, e); return; }
        if (e.button !== 0) return;

        const t = toTile(e.clientX, e.clientY);

        // If inside confirmed selection → start move drag
        if (sel && insideSel(t.x, t.y) && !isPastingMode) {
            saveHistory();
            isMoveDragging = true;
            moveStart = t;
        ...
        if (isPastingMode && clipboard) {
            saveHistory();
            pasteRegion(clipboard, t.x, t.y);
            isPastingMode = false;
            sel = { x: t.x, y: t.y, w: clipboard.w, h: clipboard.h };
            selPasteBtn.textContent = '⬇ Paste';
            return;
        }
    ...
    window.onmousemove = function(e) {
        if (activeTool !== 'select') { origMousemove && origMousemove.call(this, e); return; }
        ...
        if (isMoveDragging && sel && moveStart) {
            const t = toTile(e.clientX, e.clientY);
            movePreview = { dx: t.x - moveStart.x, dy: t.y - moveStart.y };
            return;
        }
        ...
        if (isPastingMode && clipboard) {
            const t = toTile(e.clientX, e.clientY);
            pastePreviewPos = { x: t.x, y: t.y };
            // reuse sel rect to show paste preview outline
            sel = { x: t.x, y: t.y, w: clipboard.w, h: clipboard.h };
        }

        // Cursor feedback
        if (sel && insideSel(mouseX, mouseY) && !isPastingMode) {
    ...
    window.onmouseup = function(e) {
        if (activeTool !== 'select') { origMouseup && origMouseup.call(this, e); return; }

        if (isMoveDragging && movePreview) {
            // Commit the move
            const dx = movePreview.dx, dy = movePreview.dy;
            dropLifted(dx, dy);
            sel = { x: sel.x + dx, y: sel.y + dy, w: sel.w, h: sel.h };
            // clamp
            sel.x = Math.max(0, Math.min(GRID_X - sel.w, sel.x));
            sel.y = Math.max(0, Math.min(GRID_Y - sel.h, sel.y));
            isMoveDragging = false; moveStart = null; movePreview = null;
            viewport.style.cursor = 'grab';
            showSelToolbar();
            return;
    ...
    window.onkeydown = function(e) {
        ...
        // [V] shortcut to activate select tool
        if (e.key.toLowerCase() === 'v' && !e.ctrlKey) {
            updateToolState('select'); activeTool = 'select'; return;
        }

        if (activeTool === 'select') {
            if (e.key === 'Escape') { e.preventDefault(); deselect(); return; }

            if (e.ctrlKey && e.key.toLowerCase() === 'c' && sel) {
                e.preventDefault();
                clipboard = copyRegion(sel.x, sel.y, sel.w, sel.h);
                selPasteBtn.style.display = '';
                selCopyBtn.textContent = '✔ Copied!';
            ...
            if (['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key) && sel) {
                ...
                dropLifted(ddx, ddy);
                sel = { x: sel.x + ddx, y: sel.y + ddy, w: sel.w, h: sel.h };
                sel.x = Math.max(0, Math.min(GRID_X - sel.w, sel.x));
                sel.y = Math.max(0, Math.min(GRID_Y - sel.h, sel.y));
                return;
            }
...
const FINGER_SIZE = 8; // fingerprint resolution (8×8 = 64 values × 3 channels)
const replicateFingerprints = {}; // texture path → Uint8Array (rgb, length 8*8*3)

// Render a texture image into an 8×8 RGB fingerprint
...
// Extract an 8×8 fingerprint from a region of a canvas context
...
document.getElementById('i2w-replicate-btn').onclick = () => {
    ...
    // Tolerance slider: map 10–120 range to MSE threshold
    ...
    // MSE threshold: slider 10→800, 45→3000, 120→12000
    ...
    setTimeout(() => {
        ...
        img.onload = () => {
            ...
            // Each tile in the screenshot occupies (img.width/tileW) × (img.height/tileH) pixels
            ...
            for (let ty = 0; ty < tileH; ty++) {
                for (let tx = 0; tx < tileW; tx++) {
                    ...
                    if (matchBg && bgEntries.length > 0) {
                        // For background, use a slightly looser threshold —
                        // background walls are often partially obscured by fg blocks
            ...
            drawCanvas();
            statusEl.style.color = '#4ade80';
            statusEl.innerText = `✅ Done! ${placed} blocks placed, ${skipped} tiles skipped (no match).`;
            btn.disabled = false;
            btn.innerText = '📷 Replicate World';
        };
        img.onerror = () => {
            statusEl.style.color = '#f87171';
            statusEl.innerText = '❌ Failed to load screenshot image.';

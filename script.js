...
// ============================================================
// FEATURE: Prompt to World (AI via OpenAI API)
// ============================================================

// Register the popup in the bindings system
bindings['p2w-btn'] = 'p2w-popup';
document.getElementById('p2w-btn').onclick = () => openMenu('p2w-popup');

// Persist API key in localStorage
const p2wKeyInput = document.getElementById('p2w-apikey');
p2wKeyInput.value = localStorage.getItem('p2w_apikey') || '';
p2wKeyInput.oninput = () => localStorage.setItem('p2w_apikey', p2wKeyInput.value.trim());

// Build a compact block reference string for the AI prompt
function buildBlockReference() {
    const byType = { block: [], wall: [], prop: [], water: [] };
    ASSET_LIST.forEach(b => {
        const t = b.type || 'block';
        if (byType[t]) byType[t].push(b.label);
    });
    return (
        'BLOCKS (foreground solid): ' + byType.block.join(', ') + '\n' +
        'WALLS (background decorative): ' + byType.wall.join(', ') + '\n' +
        'PROPS (decorative objects): ' + byType.prop.slice(0, 200).join(', ') + '\n' +
        'WATER (liquids): ' + byType.water.join(', ')
    );
}

// Find a block entry by label (case-insensitive, partial match fallback)
function findBlock(name) {
    if (!name) return null;
    const lower = name.toLowerCase().trim();
    let exact = ASSET_LIST.find(b => b.label.toLowerCase() === lower);
    if (exact) return exact;
    let partial = ASSET_LIST.find(b => b.label.toLowerCase().includes(lower));
    return partial || null;
}

// Apply a parsed world layout JSON to the grid
function applyAIWorldLayout(layout, startX, startY, replace) {
    saveHistory();
    let placed = 0;
    (layout.cells || []).forEach(cell => {
        const wx = startX + (cell.x || 0);
        const wy = startY + (cell.y || 0);
        if (wx < 0 || wx >= GRID_X || wy < 0 || wy >= GRID_Y) return;

        if (cell.fg) {
            const b = findBlock(cell.fg);
            if (b && (replace || !fgData[wx][wy])) {
                fgData[wx][wy] = { name: b.label, texture: b.texture || `textures/blocks/${b.folder}/${b.file}`, type: b.type, fileName: b.file };
                placed++;
            }
        }
        if (cell.bg) {
            const b = findBlock(cell.bg);
            if (b && (replace || !bgData[wx][wy])) {
                bgData[wx][wy] = { name: b.label, texture: b.texture || `textures/blocks/${b.folder}/${b.file}`, type: b.type, fileName: b.file };
                placed++;
            }
        }
    });
...
// Helper: call OpenAI and parse JSON response, with truncation recovery
async function callAIForChunk(apiKey, systemPrompt, userMsg, statusEl) {
    ...
    try {
        ...
        if (lastBrace !== -1) {
            const recovered = jsonStr.substring(0, lastBrace + 1).replace(/,\s*$/, '') + ']}';
...
document.getElementById('p2w-generate-btn').onclick = async () => {
    const apiKey   = document.getElementById('p2w-apikey').value.trim();
    const prompt   = document.getElementById('p2w-prompt').value.trim();
    const startX   = parseInt(document.getElementById('p2w-x').value) || 0;
    const startY   = parseInt(document.getElementById('p2w-y').value) || 0;
    const tileW    = Math.min(parseInt(document.getElementById('p2w-w').value) || 40, 80 - startX);
    const tileH    = Math.min(parseInt(document.getElementById('p2w-h').value) || 30, 60 - startY);
    const replace  = document.getElementById('p2w-replace').checked;
    const statusEl = document.getElementById('p2w-status');

    if (!apiKey) { statusEl.style.color = '#f87171'; statusEl.innerText = '❌ Please enter your OpenAI API key.'; return; }
    if (!prompt) { statusEl.style.color = '#f87171'; statusEl.innerText = '❌ Please enter a world description.'; return; }

    const btn = document.getElementById('p2w-generate-btn');
    btn.disabled = true;
    btn.innerText = '⏳ Generating...';
    statusEl.style.color = '#a78bfa';

    const blockRef = buildBlockReference();

    // ── Pre-pass: pick the best atmosphere background for the world ──
    const atmosphereNames = backgroundLibrary.filter(b => b.file).map(b => b.name);
    try {
        statusEl.style.color = '#a78bfa';
        statusEl.innerText = '🎨 Choosing atmosphere...';
        const atmRes = await fetch('https://api.openai.com/v1/chat/completions', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
            body: JSON.stringify({
                model: 'gpt-4o',
                ...
                temperature: 0.5,
                messages: [
                    { role: 'system', content: `You pick the best sky/atmosphere background for a Growtopia-style world. Available options: ${atmosphereNames.join(', ')}. Reply with ONLY the exact name from the list, nothing else.` },
                    { role: 'user', content: `World description: ${prompt}` }
                ]
            })
        });
        if (atmRes.ok) {
            const atmData = await atmRes.json();
            const atmName = (atmData.choices?.[0]?.message?.content || '').trim();
            const match = backgroundLibrary.find(b => b.name.toLowerCase() === atmName.toLowerCase());
            if (match) {
                saveHistory();
                setBackground(match.file);
                statusEl.innerText = `🌄 Atmosphere set to: ${match.name}`;
            }
        }
    } catch (_) { /* non-fatal — proceed without atmosphere */ }

    // Chunk size: 20 columns per request keeps each response well under token limits
    const CHUNK_COLS = 10;
    const chunks = [];
    for (let cx = 0; cx < tileW; cx += CHUNK_COLS) {
        chunks.push({ x: cx, w: Math.min(CHUNK_COLS, tileW - cx) });
    }
    ...
    try {
        for (let i = 0; i < chunks.length; i++) {
            const { x: chunkX, w: chunkW } = chunks[i];
            statusEl.innerText = `🤖 Generating chunk ${i + 1}/${totalChunks} (columns ${chunkX}–${chunkX + chunkW - 1})...`;

            const expectedCells = chunkW * tileH;
            const systemPrompt = `You are a Growtopia / PW world designer. You produce a JSON layout for a section of a larger world.

AVAILABLE BLOCKS (use EXACT names from these lists):
${blockRef}

OUTPUT FORMAT — respond with ONLY valid JSON, no markdown, no explanation:
{
  "cells": [
    { "x": 0, "y": 0, "fg": "Block Name or null", "bg": "Wall Name or null" },
    ...
  ]
}

CRITICAL RULES — FOLLOW EXACTLY:
- This chunk covers x=${chunkX} to x=${chunkX + chunkW - 1}, y=0 to y=${tileH - 1}. y=0 is top.
- x and y values in your output must be ABSOLUTE coordinates (not relative to chunk start).
- EVERY single cell in this chunk must have an entry. The chunk is ${chunkW} columns × ${tileH} rows = ${expectedCells} cells. You MUST output all ${expectedCells} cells.
- Do NOT skip or omit any cell. Every (x, y) combination in this chunk needs a JSON entry.
- fg = foreground block (solid/walkable, or null for open air). bg = background wall (null for sky).
- For open sky/air cells at the top of the world, use fg: null and bg: null (sky shows through).
- For underground/cave/interior air cells, use fg: null and a matching wall as bg (e.g. cave wall, stone background, wooden background).
- For ground cells, set fg to a solid block AND bg to a thematically matching wall.
- Only use block names from the provided lists — do not invent names.
- The world should feel fully built, not sparse. Underground and interior cells must always have a bg wall. Only true open sky cells should have bg: null.
- Make this chunk consistent with the overall world theme. Use thematically appropriate block choices (e.g. lava rocks near lava, pine logs near forests, dark blocks for caves).`;

            const userMsg = `Full world is ${tileW}×${tileH}. Design chunk x=${chunkX} to x=${chunkX + chunkW - 1} (width ${chunkW}, height ${tileH}) for: ${prompt}

REMINDER: Output ALL ${expectedCells} cells (every x from ${chunkX} to ${chunkX + chunkW - 1}, every y from 0 to ${tileH - 1}). Do not skip any cell.`;

            const layout = await callAIForChunk(apiKey, systemPrompt, userMsg, statusEl);
            const placed = applyAIWorldLayout(layout, startX, startY, replace);
            totalPlaced += placed;
            statusEl.innerText = `🧩 Chunk ${i + 1}/${totalChunks} placed (${totalPlaced} blocks so far)...`;
        }

        statusEl.style.color = '#4ade80';
        statusEl.innerText = `✅ World generated! ${totalPlaced} blocks placed across ${totalChunks} chunks.`;
    } catch (err) {

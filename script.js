...
document.getElementById('i2w-replicate-btn').onclick = async () => {
    ...
    // Yield helper — keeps UI responsive between rows
    ...
    for (let ty = 0; ty < tileH; ty++) {
        // Yield every row — this is what prevents the post-completion lag
        await yieldFrame();
        ...
        for (let tx = 0; tx < tileW; tx++) {
            ...
            if (fgMatch) {
                ...
                fgData[wx][wy] = { name: blk.name, texture: blk.texture, type: blk.type, fileName: blk.fileName, folder: blk.folder };
                placed++;
            } else {
                fgData[wx][wy] = null;
                skipped++;
            ...
            if (matchBg && bgEntries.length > 0) {
                ...
                if (bgMatch) {
                    ...
                    bgData[wx][wy] = { name: blk.name, texture: blk.texture, type: blk.type, fileName: blk.fileName, folder: blk.folder };
                    placed++;
                }
            }
        }
...
function placeBlock(palette, r, g, b, wx, wy, layer) {
    ...
    const entry = { name: blk.name, texture: blk.texture, type: blk.type, fileName: blk.fileName, folder: blk.folder };

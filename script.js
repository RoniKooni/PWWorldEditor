...
function render(time) {
    ...
    // ── Pass 1: Draw BG (wall) blocks — dimmed to suggest depth behind the wall ──
    for (let x = 0; x < GRID_X; x++) {
        for (let y = 0; y < GRID_Y; y++) {
            const bgBlock = bgData[x][y];
            if (!bgBlock) continue;

            const baseTex = getBlockTexture(x, y, bgBlock);
            if (!baseTex) continue;

            ctx.save();
            // BG blocks are "behind" the foreground wall — render them dimmed
    ...
    // ── Pass 2: Draw FG blocks on top ──
    for (let x = 0; x < GRID_X; x++) {
        for (let y = 0; y < GRID_Y; y++) {
            const fgBlock = fgData[x][y];
            if (!fgBlock) continue;

            const baseTex = getBlockTexture(x, y, fgBlock);
            if (!baseTex) continue;

            ctx.drawImage(baseTex, x * TILE, y * TILE, TILE, TILE);

            const glowName = fgBlock.fileName.replace('.png', '_Glow.png');
            const hasGlow = ASSET_LIST.some(a => a.file === glowName && a.folder === fgBlock.folder);
            if (hasGlow) {
                const glowTex = getImg(`${BASE_PATH}${fgBlock.folder}/${glowName}`);
    ...
    // ── Pass 3: Cast shadow from FG blocks onto adjacent BG blocks below/around them ──
    // In Pixel Worlds, foreground blocks cast a drop-shadow onto background tiles.
    // We approximate this by darkening BG tiles that sit directly behind an FG block.
    for (let x = 0; x < GRID_X; x++) {
        for (let y = 0; y < GRID_Y; y++) {
            if (!fgData[x][y]) continue; // only where FG exists

            // Shadow falls on the BG tile at the same position and 1 tile below/right
            const shadowTargets = [
                [x,     y    ],  // same tile — BG peeking behind FG
                [x,     y + 1],  // one row below
                [x + 1, y    ],  // one col right
            ];
            for (const [sx, sy] of shadowTargets) {
                if (sx < 0 || sx >= GRID_X || sy < 0 || sy >= GRID_Y) continue;
                if (!bgData[sx][sy]) continue;
                // Only shade BG tiles that have no FG block of their own (they're "exposed" background)
                if (sx !== x || sy !== y) {
                    if (fgData[sx][sy]) continue; // FG covers this tile anyway
                }
                ctx.save();
...
document.getElementById('screenshot-btn').onclick = () => {
    ...
    const drawBlocks = () => {
        ...
        for (let x = 0; x < GRID_X; x++) {
            for (let y = 0; y < GRID_Y; y++) {
                if (bgData[x][y]) {
                    ...
                    if (tex) {
                        tempCtx.save();
                        tempCtx.globalAlpha = 0.72;
                        tempCtx.drawImage(tex, x * TILE, y * TILE, TILE, TILE);
                        tempCtx.restore();
                    }
                }
            }
        ...
        // Pass 3: FG shadow onto BG
        for (let x = 0; x < GRID_X; x++) {
            for (let y = 0; y < GRID_Y; y++) {
                if (!fgData[x][y]) continue;
                const shadowTargets = [[x, y], [x, y + 1], [x + 1, y]];
                for (const [sx, sy] of shadowTargets) {
                    if (sx < 0 || sx >= GRID_X || sy < 0 || sy >= GRID_Y) continue;
                    if (!bgData[sx][sy]) continue;
                    if ((sx !== x || sy !== y) && fgData[sx][sy]) continue;
                    tempCtx.save();
                    tempCtx.globalAlpha = 0.30;
                    tempCtx.fillStyle = '#000';
                    tempCtx.fillRect(sx * TILE, sy * TILE, TILE, TILE);
                    tempCtx.restore();
                }
            }
        }
...
document.getElementById('i2b-variety').oninput = (e) => {
    const labels = [
        ...
        '🏛️ Pixel BG Depth — tiled backgrounds add layered depth'
    ];
...
// ─────────────────────────────────────────────
// MODE 5: PIXEL BG DEPTH — tiled background wall tiles as depth layers
// Strategy:
//   - FG layer: pixel blocks matched to image color (only for bright/midtone pixels)
//   - BG layer: background wall tiles matched to image color (all non-transparent pixels)
//   - Very dark pixels (deep shadow): BG tile only — pixel block skipped (natural depth gap)
//   - This mirrors how Pixel Worlds uses background wall tiles to add depth behind sprites
// ─────────────────────────────────────────────
function runPixelBGDepthMode(pixelData, outW, outH, startX, startY, statusEl) {
    ...
    const bgWallBlocks = blockLibrary.filter(b => {
        if (b.fileName.includes('_Alt') || b.fileName.includes('_Glow')) return false;
        const frameMatch = b.fileName.match(/_(\d+)\.png$/);
        if (frameMatch && frameMatch[1] !== '0') return false;
        return b.fileName.startsWith('PixelBackground');
    });

    if (pixelBlocks.length === 0) { statusEl.innerText = 'Error: No Pixel Blocks found!'; return; }
    if (bgWallBlocks.length === 0) { statusEl.innerText = 'Error: No background wall blocks found!'; return; }

    statusEl.innerText = '⏳ Sampling pixel block colors...';
    batchSampleBlocks(pixelBlocks, statusEl, 'Sampling pixel blocks', (fgPalette) => {
        if (fgPalette.length === 0) { statusEl.innerText = 'Error: Could not sample pixel block colors.'; return; }

        statusEl.innerText = '⏳ Sampling background tile colors...';
        batchSampleBlocks(bgWallBlocks, statusEl, 'Sampling bg tiles', (bgPalette) => {
            if (bgPalette.length === 0) { statusEl.innerText = 'Error: Could not sample background tile colors.'; return; }

            statusEl.innerText = `⚡ Placing depth layers (${fgPalette.length} fg + ${bgPalette.length} bg blocks)...`;
            saveHistory();
            ...
            for (let ty = 0; ty < outH; ty++) {
                for (let tx = 0; tx < outW; tx++) {
                    ...
                    // normLum: 0 = darkest pixel in image, 1 = brightest
                    const normLum = (lum - minLum) / lumRange;

                    const wx = startX + tx, wy = startY + ty;
                    if (wx < 0 || wx >= GRID_X || wy < 0 || wy >= GRID_Y) continue;

                    // ── BG layer: always place a wall tile (darken color slightly for depth) ──
                    const bgR = Math.round(r * 0.55), bgG = Math.round(g * 0.55), bgB = Math.round(b * 0.55);
                    const bgKey = `${bgR >> 2},${bgG >> 2},${bgB >> 2}`;
                    if (!bgCache[bgKey]) {
                        const best = findClosestBlock(bgR, bgG, bgB, bgPalette);
                        bgCache[bgKey] = best ? best.block : null;
                    }
                    if (bgCache[bgKey]) {
                        bgData[wx][wy] = JSON.parse(JSON.stringify(bgCache[bgKey]));
                        placed++;
                    }

                    // ── FG layer: pixel block only for bright/midtone pixels ──
                    // Deep shadows (normLum < 0.20) get no FG — the BG tile shows through = depth
                    if (normLum < 0.20) continue;

                    // Shade FG pixel block to match darkness
                    const shadeMult = 0.60 + normLum * 0.40; // range 0.60–1.00
                    const fgR = Math.round(r * shadeMult), fgG = Math.round(g * shadeMult), fgB = Math.round(b * shadeMult);
                    const fgKey = `${fgR >> 2},${fgG >> 2},${fgB >> 2}`;
                    if (!fgCache[fgKey]) {
                        const best = findClosestBlock(fgR, fgG, fgB, fgPalette);
                        fgCache[fgKey] = best ? best.block : null;
                    }
                    if (fgCache[fgKey]) {
                        fgData[wx][wy] = JSON.parse(JSON.stringify(fgCache[fgKey]));
                        placed++;
                    }
                }
            }
            statusEl.innerText = `✅ BG Depth done! ${placed} blocks placed. Dark areas show bg tile texture through.`;
        });
    });
}
...
document.getElementById('img2blocks-convert-btn').onclick = () => {
    ...
    tempImg.onload = () => {
        ...
        if (variety === 1) {
            ...
            // ── MODE 5: Pixel BG Depth (pixel blocks FG + bg wall tiles BG) ──
            statusEl.innerText = '⏳ BG Depth mode: sampling palettes...';
            runPixelBGDepthMode(pixelData, outW, outH, startX, startY, statusEl);
        } else {

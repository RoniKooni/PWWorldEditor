...
- x and y values in your output must be ABSOLUTE (not relative to chunk start).
- Only include cells that have at least one block. Empty cells can be omitted.
- fg = foreground block (solid/walkable). bg = background wall. Use null if empty for that layer.
- Only use block names from the provided lists — do not invent names.
- Make this chunk consistent with the overall world theme described by the user.
- Aim to fill the chunk meaningfully; don't leave it mostly empty.`;

            const userMsg = `Full world is ${tileW}×${tileH}. Design chunk x=${chunkX} to x=${chunkX + chunkW - 1} (width ${chunkW}, height ${tileH}) for this world: ${prompt}`;

...
function initUI() {
    ...
    blockLibrary.forEach(b => {
        ...
        // ── Skip blocks whose texture file doesn't exist ──
        // We detect this by checking if the image loads; for the inventory we
        // use a sync check via a pre-loaded image's naturalWidth (0 = broken).
        const img = getImg(b.texture);
        // If the image already failed (naturalWidth===0 and complete), skip it.
        // For not-yet-loaded images we optimistically include and hide on error.
        ...
        invBtn.querySelector('img').onerror = () => { invBtn.style.display = 'none'; };
        invBtn.onclick = () => {
        ...
        const makeSimpleBtn = (callback) => {
            ...
            btn.querySelector('img').onerror = () => { btn.style.display = 'none'; };
        ...
        suggest.querySelector('img').onerror = () => { suggest.style.display = 'none'; };
        suggest.onclick = () => {

...
(function () {
    ...
    viewport.onmousedown = function(e) {
        ...
        if (activeTool === 'arrange') {
            if (getRotateHandleAt(t.x, t.y) && sel && !activeLayerLocked()) {
                saveHistory();
                liftSelection();
                rotateStart = { x: sel.x, y: sel.y, w: sel.w, h: sel.h };
                rotateSource = { fg: liftedFg, bg: liftedBg, w: sel.w, h: sel.h };
                rotatePreview = getRotatedRect(rotateSource, 0);
    ...
    window.onmousemove = function(e) {
        ...
        if (activeTool === 'arrange' && isRotateDragging && rotateStart && rotateSource) {
            const t = toTile(e.clientX, e.clientY);
            rotatePreview = getRotatedRect(rotateSource, getRotateTurns(t));
            viewport.style.cursor = canPlaceArrangeRegion(rotatePreview.data, rotatePreview.x, rotatePreview.y) ? 'grabbing' : 'not-allowed';
            return;
        }

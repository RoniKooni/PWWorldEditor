...
document.getElementById('p2w-generate-btn').onclick = async () => {
    ...
    statusEl.innerText = '🤖 Asking AI to design your world...';

    const blockRef = buildBlockReference();

    const systemPrompt = `You are a Growtopia / PW world designer. Given a description and a grid size, you produce a JSON layout.
...
Rules:
- x ranges 0 to ${tileW - 1}, y ranges 0 to ${tileH - 1}. y=0 is top.
- Only include cells that have at least one block (fg or bg). Empty cells can be omitted.
- fg = foreground block (solid, walkable) — use type "block" or "prop" names.
- bg = background wall — use type "wall" names.
- Use null if no block for that layer.
- Only use block names from the provided lists — do not invent names.
- Create a visually interesting, layered world that matches the description.
- Ground/floor should use solid blocks. Sky/background areas use walls. Decorative items use props.
- Aim to fill the grid meaningfully; don't leave it mostly empty.`;

    const userMsg = `Design a ${tileW}×${tileH} tile world: ${prompt}`;
    ...
    try {
        ...
        statusEl.innerText = '🧩 Parsing layout...';
        ...
        try {
            ...
            if (match) layout = JSON.parse(match[0]);
